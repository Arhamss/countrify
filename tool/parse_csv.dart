...
void main() async {
  ...
  for (final row in countries) {
    ...
    final name = row[0].toString().trim().replaceAll("'", "\\'");           // Country
    final abbreviation = row[1].toString().trim();                         // Abbreviation
    final callingCode = row[2].toString().trim();                          // Calling Code
    final capital = row[3].toString().trim().replaceAll("'", "\\'");       // Capital/Major City
    final currencyCode = row[4].toString().trim();                         // Currency-Code
    final largestCity = row[5].toString().trim().replaceAll("'", "\\'");   // Largest city
    final language = row[6].toString().trim().replaceAll("'", "\\'");      // Official language
    final population = row[7].toString().trim().replaceAll(',', '');       // Population
    ...
    buffer.writeln("      flagImagePath: 'packages/countrify/assets/images/flags/$abbreviation.png',");
    ...
    buffer.writeln("      population: ${population.isNotEmpty ? population : '0'},");
    buffer.writeln('      area: 0.0, // Not available in CSV');
    buffer.writeln("      callingCodes: ['$callingCode'],");
    buffer.writeln("      topLevelDomains: ['.${abbreviation.toLowerCase()}'],");
    buffer.writeln("      currencies: [Currency(code: '$currencyCode', name: '$currencyCode', symbol: '')],");
    buffer.writeln("      languages: [Language(iso6391: 'en', iso6392: 'eng', name: '$language', nativeName: '$language')],");
  ...
  ..writeln('  static List<Country> get all => List.unmodifiable(_allCountries);')
  ..writeln()
  ..writeln('  /// Get countries by region')
  ..writeln('  static List<Country> getByRegion(String region) {')
  ..writeln('    return _allCountries.where((country) => country.region == region).toList();')
  ..writeln('  }')
  ..writeln()
  ..writeln('  /// Get countries by subregion')
  ..writeln('  static List<Country> getBySubregion(String subregion) {')
  ..writeln('    return _allCountries.where((country) => country.subregion == subregion).toList();')
  ..writeln('  }')
  ..writeln()
  ..writeln('  /// Get country by alpha-2 code')
  ..writeln('  static Country? getByAlpha2Code(String alpha2Code) {')
  ..writeln('    try {')
  ..writeln('      return _allCountries.firstWhere((country) => country.alpha2Code == alpha2Code.toUpperCase());')
  ..writeln('    } catch (e) {')
  ..writeln('      return null;')
  ..writeln('    }')
  ..writeln('  }')
  ...
  ..writeln('    try {')
  ..writeln('      return _allCountries.firstWhere((country) => country.alpha3Code == alpha3Code.toUpperCase());')
  ..writeln('    } catch (e) {')
  ..writeln('      return null;')
  ..writeln('    }')
  ..writeln('  }')
  ...
  ..writeln('    try {')
  ..writeln('      return _allCountries.firstWhere((country) => country.numericCode == numericCode);')
  ..writeln('    } catch (e) {')
  ..writeln('      return null;')
  ..writeln('    }')
  ..writeln('  }')
  ...
  ..writeln('      return country.name.toLowerCase().contains(lowercaseQuery) ||')
  ..writeln('          country.nameTranslations.values.any((translation) => ')
  ..writeln('              translation.toLowerCase().contains(lowercaseQuery));')
  ..writeln('    }).toList();')
  ..writeln('  }')
  ..writeln()
  ..writeln('  /// Get independent countries only')
  ..writeln('  static List<Country> get independent => ')
  ..writeln('      _allCountries.where((country) => country.isIndependent).toList();')
  ..writeln()
  ..writeln('  /// Get UN member countries only')
  ..writeln('  static List<Country> get unMembers => ')
  ..writeln('      _allCountries.where((country) => country.isUnMember).toList();')
  ..writeln('}');
...
String _getAlpha3Code(String alpha2Code) {
  ...
  final alpha3Map = {
    ...
    'CN': 'CHN', 'CX': 'CXR', 'CC': 'CCK', 'CO': 'COL', 'KM': 'COM', 'CG': 'COG',
    'CD': 'COD', 'CK': 'COK', 'CR': 'CRI', 'CI': 'CIV', 'HR': 'HRV', 'CU': 'CUB',
    'CY': 'CYP', 'CZ': 'CZE', 'DK': 'DNK', 'DJ': 'DJI', 'DM': 'DMA', 'DO': 'DOM',
    'EC': 'ECU', 'EG': 'EGY', 'SV': 'SLV', 'GQ': 'GNQ', 'ER': 'ERI', 'EE': 'EST',
    'ET': 'ETH', 'FK': 'FLK', 'FO': 'FRO', 'FJ': 'FJI', 'FI': 'FIN', 'FR': 'FRA',
    'GF': 'GUF', 'PF': 'PYF', 'TF': 'ATF', 'GA': 'GAB', 'GM': 'GMB', 'GE': 'GEO',
    'DE': 'DEU', 'GH': 'GHA', 'GI': 'GIB', 'GR': 'GRC', 'GL': 'GRL', 'GD': 'GRD',
    'GP': 'GLP', 'GU': 'GUM', 'GT': 'GTM', 'GG': 'GGY', 'GN': 'GIN', 'GW': 'GNB',
    'GY': 'GUY', 'HT': 'HTI', 'HM': 'HMD', 'VA': 'VAT', 'HN': 'HND', 'HK': 'HKG',
    'HU': 'HUN', 'IS': 'ISL', 'IN': 'IND', 'ID': 'IDN', 'IR': 'IRN', 'IQ': 'IRQ',
    'IE': 'IRL', 'IM': 'IMN', 'IL': 'ISR', 'IT': 'ITA', 'JM': 'JAM', 'JP': 'JPN',
    'JE': 'JEY', 'JO': 'JOR', 'KZ': 'KAZ', 'KE': 'KEN', 'KI': 'KIR', 'KP': 'PRK',
    'KR': 'KOR', 'KW': 'KWT', 'KG': 'KGZ', 'LA': 'LAO', 'LV': 'LVA', 'LB': 'LBN',
    'LS': 'LSO', 'LR': 'LBR', 'LY': 'LBY', 'LI': 'LIE', 'LT': 'LTU', 'LU': 'LUX',
    'MO': 'MAC', 'MK': 'MKD', 'MG': 'MDG', 'MW': 'MWI', 'MY': 'MYS', 'MV': 'MDV',
    'ML': 'MLI', 'MT': 'MLT', 'MH': 'MHL', 'MQ': 'MTQ', 'MR': 'MRT', 'MU': 'MUS',
    'YT': 'MYT', 'MX': 'MEX', 'FM': 'FSM', 'MD': 'MDA', 'MC': 'MCO', 'MN': 'MNG',
    'ME': 'MNE', 'MS': 'MSR', 'MA': 'MAR', 'MZ': 'MOZ', 'MM': 'MMR', 'NA': 'NAM',
    'NR': 'NRU', 'NP': 'NPL', 'NL': 'NLD', 'NC': 'NCL', 'NZ': 'NZL', 'NI': 'NIC',
    'NE': 'NER', 'NG': 'NGA', 'NU': 'NIU', 'NF': 'NFK', 'MP': 'MNP', 'NO': 'NOR',
    ...
    'BL': 'BLM', 'SH': 'SHN', 'KN': 'KNA', 'LC': 'LCA', 'MF': 'MAF', 'PM': 'SPM',
    'VC': 'VCT', 'WS': 'WSM', 'SM': 'SMR', 'ST': 'STP', 'SA': 'SAU', 'SN': 'SEN',
    'RS': 'SRB', 'SC': 'SYC', 'SL': 'SLE', 'SG': 'SGP', 'SX': 'SXM', 'SK': 'SVK',
    'SI': 'SVN', 'SB': 'SLB', 'SO': 'SOM', 'ZA': 'ZAF', 'GS': 'SGS', 'SS': 'SSD',
    ...
    'UZ': 'UZB', 'VU': 'VUT', 'VE': 'VEN', 'VN': 'VNM', 'VI': 'VIR', 'WF': 'WLF',
...
String _getNumericCode(String alpha2Code) {
  ...
  final numericMap = {
    ...
    'BT': '064', 'BO': '068', 'BA': '070', 'BW': '072', 'BR': '076', 'VG': '092',
    'BN': '096', 'BG': '100', 'BF': '854', 'BI': '108', 'KH': '116', 'CM': '120',
    'CA': '124', 'CV': '132', 'KY': '136', 'CF': '140', 'TD': '148', 'CL': '152',
    'CN': '156', 'CX': '162', 'CC': '166', 'CO': '170', 'KM': '174', 'CG': '178',
    'CD': '180', 'CK': '184', 'CR': '188', 'CI': '384', 'HR': '191', 'CU': '192',
    'CY': '196', 'CZ': '203', 'DK': '208', 'DJ': '262', 'DM': '212', 'DO': '214',
    'EC': '218', 'EG': '818', 'SV': '222', 'GQ': '226', 'ER': '232', 'EE': '233',
    'ET': '231', 'FK': '238', 'FO': '234', 'FJ': '242', 'FI': '246', 'FR': '250',
    'GF': '254', 'PF': '258', 'TF': '260', 'GA': '266', 'GM': '270', 'GE': '268',
    'DE': '276', 'GH': '288', 'GI': '292', 'GR': '300', 'GL': '304', 'GD': '308',
    'GP': '312', 'GU': '316', 'GT': '320', 'GG': '831', 'GN': '324', 'GW': '624',
    'GY': '328', 'HT': '332', 'HM': '334', 'VA': '336', 'HN': '340', 'HK': '344',
    'HU': '348', 'IS': '352', 'IN': '356', 'ID': '360', 'IR': '364', 'IQ': '368',
    'IE': '372', 'IM': '833', 'IL': '376', 'IT': '380', 'JM': '388', 'JP': '392',
    'JE': '832', 'JO': '400', 'KZ': '398', 'KE': '404', 'KI': '296', 'KP': '408',
    'KR': '410', 'KW': '414', 'KG': '417', 'LA': '418', 'LV': '428', 'LB': '422',
    'LS': '426', 'LR': '430', 'LY': '434', 'LI': '438', 'LT': '440', 'LU': '442',
    'MO': '446', 'MK': '807', 'MG': '450', 'MW': '454', 'MY': '458', 'MV': '462',
    'ML': '466', 'MT': '470', 'MH': '584', 'MQ': '474', 'MR': '478', 'MU': '480',
    'YT': '175', 'MX': '484', 'FM': '583', 'MD': '498', 'MC': '492', 'MN': '496',
    'ME': '499', 'MS': '500', 'MA': '504', 'MZ': '508', 'MM': '104', 'NA': '516',
    'NR': '520', 'NP': '524', 'NL': '528', 'NC': '540', 'NZ': '554', 'NI': '558',
    'NE': '562', 'NG': '566', 'NU': '570', 'NF': '574', 'MP': '580', 'NO': '578',
    'OM': '512', 'PK': '586', 'PW': '585', 'PS': '275', 'PA': '591', 'PG': '598',
    'PY': '600', 'PE': '604', 'PH': '608', 'PN': '612', 'PL': '616', 'PT': '620',
    'PR': '630', 'QA': '634', 'RE': '638', 'RO': '642', 'RU': '643', 'RW': '646',
    'BL': '652', 'SH': '654', 'KN': '659', 'LC': '662', 'MF': '663', 'PM': '666',
    'VC': '670', 'WS': '882', 'SM': '674', 'ST': '678', 'SA': '682', 'SN': '686',
    'RS': '688', 'SC': '690', 'SL': '694', 'SG': '702', 'SX': '534', 'SK': '703',
    ...
    'UZ': '860', 'VU': '548', 'VE': '862', 'VN': '704', 'VI': '850', 'WF': '876',
